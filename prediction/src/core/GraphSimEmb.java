...
public class GraphSimEmb extends GraphSim {
	...
	public class F implements DiffFunc<Example> {
		...
		protected double rawValueAt(Example ex, double[] w) {
			double pos = Tools.relu(computePair(ex.qnF, null, null, w));
			double neg = Tools.relu(computePair(ex.qmF, null, null, w));
			return pos - neg;
		}
		...
		protected double[] getDelta(Feature qnF, double[] w) {
			double v = computePair(qnF, null, null, w);
			double[] result = new double[w.length];
						
			for (int i = 0; i < w.length; i++) 
				result[i] = qnF.get(i) * (v > 0 ? 1 : 0);
	...
	public GraphSimEmb(FeatureDB fdb) {
		super(fdb);
	}
